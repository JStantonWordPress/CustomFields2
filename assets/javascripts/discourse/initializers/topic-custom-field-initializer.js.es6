...
export default {
  ...
  initialize(container) {
    ...
    withPluginApi('0.11.2', api => {
      ...
      api.registerConnectorClass('composer-fields', 'composer-topic-custom-field-container3', {
        setupComponent(attrs, component) {
          const model = attrs.model;

          // If the first post is being edited we need to pass our value from
          // the topic model to the composer model.
          if (!isDefined(model[fieldName3]) && model.topic && model.topic[fieldName3]) {
            model.set(fieldName3, model.topic[fieldName3]);
          }

          let props = {
            fieldName3: fieldName3,
            fieldValue3: model.get(fieldName3)
          }
          component.setProperties(Object.assign(props, fieldInputTypes(fieldType)));
        },

        actions: {
          onChangeField(fieldValue3) {
            this.set(`model.${fieldName3}`, fieldValue3);
          }
        }
      });
      ...
      api.registerConnectorClass('edit-topic', 'edit-topic-custom-field-container3', {
        setupComponent(attrs, component) {
          const model = attrs.model;
        ...
        actions: {
          onChangeField(fieldValue3) {
            this.set(`buffered.${fieldName3}`, fieldValue3);
          }
        }
      });
      ...
      api.serializeOnCreate(fieldName3);
      api.serializeToDraft(fieldName3);
      api.serializeToTopic(fieldName3, `topic.${fieldName3}`);
      ...
      api.modifyClass('component:topic-list-item', {
        ...
        customFieldName3: fieldName3,
        customFieldValue3: alias(`topic.${fieldName3}`),
